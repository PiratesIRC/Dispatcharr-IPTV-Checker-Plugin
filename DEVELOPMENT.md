...
CHANGELOG.md                   # consolidated changelog (seeded from the RELEASE_NOTES files 2026-09-06)
scripts/bump_version.py        # next UTC calver into plugin.json, plugin.py and CLAUDE.md + changelog stub
scripts/check_version_sync.py  # standalone version-drift check (CI/pre-commit usable)
scripts/validate_zip.py        # release zip guard: separators, package root, dev files, CRLF
scripts/build_release_zip.py   # git archive from the index (LF forced) then validate
...
```bash
python scripts/bump_version.py            # auto, from current UTC time
python scripts/bump_version.py --set 1.26.1621430  # explicit
```
...
**Hooks**: the workspace-level hooks (compile any edited `.py`, validate
`plugin.json`, block container lifecycle commands) cover this repository; the
two project hooks that used to live under `.claude/hooks/` were retired on
2026-09-06. The git pre-commit hook in `.githooks/` runs `check_version_sync.py`,
so plugin.json, plugin.py and the CLAUDE.md version line cannot drift.

**Skills**: `release`, `pre-release-qa` and `deploy-plugin` are workspace-level
skills parameterised by `release.json`. `/triage-scheduler` (this repository)
is the step-by-step diagnosis of scheduler election, stale progress,
pending-resume, and reload-flag state in the container.
